...
public final class LollipopDualModeSupport {
  ...
  private static long getSubId(final int cardIndex) throws ReflectHiddenFuncException {
    try {
      Field field;
      if (cardIndex == 0) {
      ...
      method.setAccessible(true);
      final long[] val = (long[]) method.invoke(null, slotId);
      return val[0];
  ...
  public static String getSubscriberId(final Context context, final int cardIndex) throws ReflectHiddenFuncException {
    try {
      final Method method = TelephonyManager.class.getDeclaredMethod(EncryptUtils.decode(CommConstant.GETSUBSCRIBERID_TAG), Long.TYPE);
      method.setAccessible(true);
      final TelephonyManager tm = (TelephonyManager) context.getSystemService(EncryptUtils.decode(CommConstant.PHONE_TAG));
  ...
  private static SmsManager getSmsManager(final int cardIndex) throws ReflectHiddenFuncException {
    try {
      final Method method = SmsManager.class.getDeclaredMethod(EncryptUtils.decode(CommConstant.GETSMSMANAGERFORSUBSCRIBER_TAG), Long.TYPE);
      method.setAccessible(true);
